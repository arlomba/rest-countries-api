...
const transformCountryFields = (countries)=>{
    return countries.map((country)=>{
        const { name , flag , population , capital , region , topLevelDomain , nativeName , alpha3Code , subregion , languages , currencies , borders ,  } = country;
        const tld1 = topLevelDomain.map((tld)=>tld
        ).join(" ");
        return {
            name,
            flag,
            ...
            currencies: currencies ? currencies.map((c)=>c.name
            ).join(", ") : "¯\\_(ツ)_/¯",
            borders
        };
    });
...
const createBorderTemplate = (border)=>{
    ...
    countryBorder.classList.add("border-2", "border-gray-500", "shadow", "py-2", "px-4", "dark:border-0", "dark:bg-blue-700");
...
const setCountryDetails = (c)=>{
    ...
    if (c.borders) c.borders.forEach((border)=>{
        borderCountry.appendChild(createBorderTemplate(border));
    });
    else borderCountry.textContent = "¯\\_(ツ)_/¯";
